...
func Init() {
	go func() {
		for{
			time.Sleep(time.Duration(Source.EmailExpire) * time.Second)
			Lock.Lock()
			if len(FormDataHistory) > 0 {
				send()
				FormDataHistory = make([]*FromData,0)
			}
			Lock.Unlock()
		}
	}()
...
func InitEmail(ep *EmailParam) {
	...
	m.SetAddressHeader("From", fromEmail, "山西转型综合改革示范区金融服务平台")
}
